...
const VALID_OPTIONS = Object.freeze(['properties', 'silenceErrors', 'logErrors', 'logFunction']);
	...
	set(defineVariables) {
		try {
			...
			Object.entries(defineVariables).forEach(([envVarKey, appVarOptions]) => {
				const prop = new ConfigProperty(envVarKey, appVarOptions);
				this._properties.push(prop);
			});
		} catch (error) {
			throw new configErrors.Error(
				'Could not define app config properties. Invalid variable options.',
				error
			);
		}
	},
...
ConfigService.prototype.findOne = function (name = '') {
	// , silenceError = false) {
	const config = this.properties;
	const found = config.find((prop) => prop.isMatch(name));
	console.debug(`\n\nIN findOne(${name}) = `);
	console.debug(found);
	return found;
};

ConfigService.prototype.findSeveral = function (names = '') {
	// , silenceError = false) {
...
ConfigService.prototype.get = function (name = '') {
	...
	switch (true) {
		...
		default:
			...
			throw new configErrors.Error(
				'Unable to get config variable',
				new TypeError(`app config#getVariable was called with invalid param ${typeof name}`)
			);
	}
};
...
ConfigService.prototype.toString = function (propArray = []) {
	...
	const configArray = propArray || this._properties;
